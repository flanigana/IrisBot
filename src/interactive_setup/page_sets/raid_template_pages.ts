...
export default function addRaidTemplatePages(
	pageSet: PageSet<IRaidTemplate>,
	template: Partial<IRaidTemplate>,
	guildId: string
): void {
	const raidTemplateService = container.get<RaidTemplateService>(RaidTemplateService);
		...
		new DynamicPage<IRaidTemplate>(
			...
			async (fields: Partial<IRaidTemplate>): Promise<MessageEmbed> => {
				...
				ClientTools.addFieldsToEmbed(
					...
					{
						name: 'Other Template Names',
						value: existingTemplates,
		...
		new DynamicPage<IRaidTemplate>(
			...
			(fields: Partial<IRaidTemplate>): MessageEmbed => {
				const embed = clientTools
					...
					.setDescription(
						'Respond with the description you would like displayed when you use this raid template. ' +
				...
				ClientTools.addFieldsToEmbed(
					...
					ClientTools.LINE_BREAK_FIELD,
					{
						name: 'Example',
						value:
							'`This will be a Void. Please react with the appropriate class/ability reactions. ' +
		...
		new DynamicPage<IRaidTemplate>(
			...
			(fields: Partial<IRaidTemplate>): MessageEmbed => {
				const embed = clientTools
					...
					.setDescription(
						'Respond with the emoji you would like raiders to react with to be counted in the raid. ' +
							'\n\nBelow is a list of emojis available for use.' +
							'\n**Emoji Note:** To use any emoji that is ***not*** in **this** guild but ***is*** in the following list, you ***must*** type the name as `<emojiname>`.'
					);
				ClientTools.addEmojiListToEmbed(embed, availableEmojiList);
		...
		new DynamicPage<IRaidTemplate>(
			...
			(fields: IRaidTemplate): MessageEmbed => {
				const embed = clientTools
					...
					.setDescription(
						'Respond with the emojis you would like raiders to react with to be confirmed by the bot. Respond with an emoji *and* ' +
				...
				ClientTools.addFieldsToEmbed(
					...
					{
						name: 'Example',
						value: '`<losthallskey>` or `<losthallskey> 3` or `<losthallskey> 1 <vialkey> 3`',
					},
					{ name: 'To Remove All', value: '`--ALL`' },
					{
						name: 'Secondary Reacts: Limits',
						value: raidReactsToStringArray(fields.secondaryReacts),
			...
			(fields: IRaidTemplate, res: string): string => {
				...
				for (const arg of args) {
					...
					if (!isNaN(num)) {
						// is num
						if (!emoji) {
							// if num did not come after emoji
						...
						if (emoji) {
							// if last arg was emoji, add with 0 limit
		...
		new DynamicPage<IRaidTemplate>(
			...
			(fields: Partial<IRaidTemplate>): MessageEmbed => {
				const embed = clientTools
					...
					.setDescription(
						'Respond with any additional emojis you would like raiders to react with. These have no use by the bot ' +
				...
				ClientTools.addFieldsToEmbed(
					...
					{
						name: 'Example',
						value: '`<warriorclass> <knightclass> <paladinclass> <ogmurability> <marblesealability>`',
					},
					{
						name: 'Template Additional Reacts',
						value: fields.additionalReacts.map((r) => r.react),
