...
import { DungeonCompletions, RealmEyeUserData } from "../realmeye/realmeye_types";
...
export class VerificationManager {
     ...
     */
    private failTemplateVerification(user: GuildMember, userData: RealmEyeUserData, template: IVerificationTemplate, status: Status<any>): void {
     ...
     * @param template VerificationTemplate User is verifying for
     */
    private getRankRole(user: GuildMember, userData: RealmEyeUserData, template: IVerificationTemplate): Role {
    ...
    private removeVerificationRoles(user: GuildMember, template: IVerificationTemplate): Promise<Status<any>> {
        ...
        if (template.guildRoles?.setRoles) {
            ...
            removedRoles.push(...RolesAndChannels.getRoles(user.guild, founderRole, leaderRole, officerRole, memberRole, initiateRole));
        }
     ...
     * @param template VerificationTemplate User is verifying for
     */
    private async updateUserNicknameAndRoles(user: GuildMember, userData: RealmEyeUserData, template: IVerificationTemplate): Promise<Status<GuildMember>> {
        const status = Status.createPending<GuildMember>();

        const promises = [];
        const addedRoles: Role[] = [];
        const removedRoles: Role[] = [];
        if (template.guildVerification && template.guildRoles.setRoles && StringUtils.equalsIgnoreCase(userData.guild, template.guildName)) {
        ...
        if (user.manageable) {
            ...
            promises.push(user.roles.add(addedRoles).then(u => {
     ...
     * @param template VerificationTemplate User is verifying for
     */
    private async completeTemplateVerification(user: GuildMember, userData: RealmEyeUserData, template: IVerificationTemplate, manualOverride?: ManualOverride): Promise<void> {
        ...
        await this._VerificationService.updateVerificationStatus(user.id, template._id, manualOverride ? VerificationStatus.MANUALLY_VERIFIED : VerificationStatus.VERIFIED);
     ...
     */
    private verifyDungeonRequirements(userData: RealmEyeUserData, dungeonRequirements: DungeonCompletions): Status<any> {
        ...
        if (!StringUtils.isEmpty(failureReason)) {
            if (Object.keys(userData.dungeonCompletions).length === 0) {
                failureReason += 'Hint: Your graveyard may be set to private.\n' +
     ...
     */
    private verifyRequirements(userData: RealmEyeUserData, template: IVerificationTemplate): Status<any> {
        const status = Status.createPending();

        if (userData.rank < template.rank) {
            status.addFailureReason(
                {failure: 'Rank Too Low', expected: template.rank, actual: userData.rank}
            );
        }
        if (userData.fame < template.fame) {
            status.addFailureReason(
                {failure: 'Insufficient Fame', expected: template.fame, actual: userData.fame}
            );
        }
        if (template.requireHidden && !StringUtils.equalsIgnoreCase(userData.lastSeen, 'hidden')) {
            status.addFailureReason(
                {failure: 'Hidden Location Required', failureMessage: 'Hidden location is required and yours is not.\n' +
     ...
     * @param template Template user is attempting verification for
     */
    private verifyUserInTemplateGuild(userData: RealmEyeUserData, template: IVerificationTemplate): Status<any> {
        const status = Status.createPending();
        if (!template.guildVerification) {
            return status.finalize();
        }
        if (StringUtils.equalsIgnoreCase(userData.guild, template.guildName)) {
            status.addFailureReason(
                {failure: 'Not in Guild', expected: template.guildName, actual: userData.guild}
            );
        }
        return status.finalize();
    }
     ...
     */
    public async beginTemplateVerification(user: User, template: IVerificationTemplate, manualOverride?: ManualOverride): Promise<void> {
        ...
        try {
            ...
            if (error instanceof RealmEyeError) {
                this._VerificationMessenger.sendGeneralVerificationFailureToUser(
                    user,
                    {name: 'RealmEye Error', value: error.message}
                );
            } else {
                Logger.error('Unexpected error during template verification', {error: error, user: user});
                this._VerificationMessenger.sendGeneralVerificationFailureToUser(
                    user,
                    {name: 'Unexpected Error', value: 'An unexpected error occurred during verification attempt.'}
                );
            }
        }
    }
    ...
    private async processQueuedVerifications(user: User): Promise<void> {
        const queuedVerifications = await this._VerificationService.findByUserIdAndStatus(user.id, VerificationStatus.QUEUED)
     ...
     */
    private async completeIgnLinking(user: User, userData: RealmEyeUserData): Promise<void> {
        ...
        try {
            RealmEyeService.getRealmEyeUserData(ign).then(async userData => {
                if (!userData) {
                    ...
                    this._VerificationMessenger.sendGeneralVerificationFailureToUser(
                        user,
                        {name: 'Unexpected Error Occurred', value: 'An unexpected error caused verification to fail. Please try again.\n' +
                ...
                if (userData.description?.includes(`IrisBot_${vericode._id}`)) {
                    ...
                    if (await this._UserService.existsByIgn(userData.name)) {
                        ...
                        this._VerificationMessenger.sendGeneralVerificationFailureToUser(
                            user,
                            {name: 'IGN Linked to Another User', value: `The IGN, [${ign}](${userData.realmEyeUrl}), is already registered to a Discord user. If you believe this is a mistake, reach out to the bot developer.`});
                        return;
                    }
            ...
            if (error instanceof RealmEyeError) {
                ...
                Logger.error('Unexcepted error during attempted ign verification', {error: error, user: user, ign: ign});
            }
        }
    }
     ...
     */
    private async checkUserIsVerifiable(user: User, template: IVerificationTemplate): Promise<Status<VerificationStatus>> {
        const status = Status.createPending<VerificationStatus>();
        if (!template) {
            status.addFailureReason({failure: 'Template Does Not Exist', failureMessage: 'Recieved undefined template'});
            return status.finalize();
        }
        ...
        switch (verification.status) {
            ...
            case VerificationStatus.BANNED:
                status.addFailureReason({failure: 'Unverifiable', failureMessage: `${user} is ${verification.status}`});
                return status.finalize();
     ...
     * @returns 
     */
    public async beginManualUnverification(message: Message, verifyingUser: GuildMember, template: IVerificationTemplate): Promise<void> {
        this._VerificationService.updateVerificationStatus(verifyingUser.id, template._id, VerificationStatus.UNVERIFIED);
     ...
     * @returns 
     */
    public async beginManualVerification(message: Message, verifyingUser: User, template: IVerificationTemplate): Promise<void> {
        ...
        if (await this._UserService.hasVerifiedIgn(verifyingUser.id)) {
            ...
            this._VerificationMessenger.sendGeneralVerificationFailureToGuild(
                message.channel,
                {name: 'User IGN Not Linked', value: `${verifyingUser} has not linked their IGN with Iris Bot yet. `
    ...
    public async beginManualCommand(message: Message, args: string[], command: 'verify' | 'unverify'): Promise<void> {
        ...
        const template = await this._VerificationTemplateService.findTemplateByGuildIdAndName(message.guild.id, args[2], false);
        if (!template) {
            Logger.debug('No VerificationTemplate found during attempted manual verification... Aborting verification', {guild: message.guild, args: args});
            this._VerificationMessenger.sendGeneralVerificationFailureToGuild(
                message.channel,
                {name: 'Template Not Found', value: `Template with name, ${args[2]}, not found in guild.`}
            );
            return;
        }
        ...
        if (!userId || !verifyingUser) {
            ...
            this._VerificationMessenger.sendGeneralVerificationFailureToGuild(
                message.channel,
                {name: 'Invalid User', value: `No user matching ${args[1]} could be found in this guild.`}
            );
            return;
        }
    ...
    public async beginVerification(message: Message): Promise<void> {
        ...
        if (!template) {
            Logger.warn('No VerificationTemplate found during attempted verification... Aborting verification', {guild: message.guild, channel: message.channel});
            return;
        }
